...
@section('title', 'Edit MOA Record')

@section('content')
<div class="min-h-screen py-12">
    <div class="max-w-2xl">
        <div class="mb-8">
            <h1 class="text-3xl font-semibold text-gray-900">Edit MOA Record</h1>
            <p class="text-sm text-gray-500 mt-1">Update MOA tracking information</p>
        </div>

        <div class="bg-white rounded-lg p-8 border border-gray-200">
            <form action="{{ route('moa.update', $moa) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <select name="intern_id" id="intern_id" required class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all @error('intern_id') border-red-500 @enderror">
                            ...
                            @foreach($interns as $intern)
                                <option value="{{ $intern->id }}" {{ old('intern_id', $moa->intern_id) == $intern->id ? 'selected' : '' }}>{{ $intern->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="date" name="valid_until" id="valid_until" value="{{ old('valid_until', $moa->valid_until?->format('Y-m-d')) }}" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all @error('valid_until') border-red-500 @enderror">
